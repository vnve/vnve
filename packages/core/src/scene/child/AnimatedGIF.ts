/**
 * refer from https://github.com/pixijs/gif/blob/v2.x/src/AnimatedGIF.ts
 */
import {
  Sprite,
  Texture,
 ...
 */
export class AnimatedGIF extends Sprite implements DisplayChild {
  public name: string;
  public label: string;
  public type: string;
   ...
   */
  constructor(options: Partial<AnimatedGIFOptions>) {
    super(Texture.EMPTY);
  ...
  public async load() {
    ...
    const { frames, width, height } = AnimatedGIF.fromBuffer(buffer);
    ...
    // TODO: perf增加自定义选项
    const { source, scaleMode, ...rest } = AnimatedGIF.defaultOptions;
  ...
  update(deltaTime: number): void {
    ...
    //@ts-ignore
    const currentTime = Ticker.shared.time * 1000;
  ...
  static async fromJSON(json: AnyJSON) {
    const gif = new AnimatedGIF({ source: json.source });

    gif.assetID = json.assetID;
    gif.assetType = json.assetType;

    await copyFromJSON(json, gif);
